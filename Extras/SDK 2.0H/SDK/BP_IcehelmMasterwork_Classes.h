...
namespace CG
{
	...
	public:
		...
		unsigned char                                              UnknownData_35GK[0x7];                                   // 0x02B1(0x0007) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		...
		TArray<struct FS_ResourceEnumAndWeight>                    Contents;                                                // 0x02C0(0x0010) Edit, BlueprintVisible, DisableEditOnInstance

	public:
		void Add_to_Contents(const struct FS_ResourceEnumAndWeight& Resource);
		void IsClassInOurRequirements_(class UClass* Resource, bool* IsRequired_, E_MetalType_E_MetalType* Type);
		void FindWeightNeededOfSpecific(E_MetalType_E_MetalType Type, float* Weight);
